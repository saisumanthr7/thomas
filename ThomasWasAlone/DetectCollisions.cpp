	...
	for (int x = startX; x < endX; x++)
	{
		for (int y = startY; y < endY; y++)
		{
			...
			if (m_ArrayLevel[y][x] == 2 || m_ArrayLevel[y][x] == 3)
			{
				if (character.getHead().intersects(block))
				{
					...
					if (m_ArrayLevel[y][x] == 2)// Fire, ouch!
					{
						...
						// Notify observers for fire collision
						character.notifyObservers(EventType::CollisionWithFire); 
					...
					else // Water
					{
						...
						// Notify observers for ice collision
						character.notifyObservers(EventType::CollisionWithWater);
					}
				}
			}
			...
			if (m_ArrayLevel[y][x] == 4)
			{
				...
				if (character.getFeet().intersects(block)) {
					reachedGoal = true;
					//character.notifyObservers(EventType::LevelComplete); // Notify observers
				}
			}
			// Rubber tile
